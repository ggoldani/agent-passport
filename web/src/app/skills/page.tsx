  ...
  while (i < lines.length) {
    ...
    if (/^\|/.test(lines[i])) {
      ...
      if (isSeparator(tableLines[1])) {
        headerHtml = `<thead><tr>${parseRow(tableLines[0]).map((c) => `<th>${c}</th>`).join("")}</tr></thead>`;
        for (let r = 2; r < tableLines.length; r++) {
          if (!isSeparator(tableLines[r])) {
            bodyHtml += `<tr>${parseRow(tableLines[r]).map((c) => `<td>${c}</td>`).join("")}</tr>`;
          }
        }
        ...
        for (const tl of tableLines) {
          if (!isSeparator(tl)) {
            bodyHtml += `<tr>${parseRow(tl).map((c) => `<td>${c}</td>`).join("")}</tr>`;
          }
        }
        bodyHtml = `<tbody>${bodyHtml}</tbody>`;
      }
...
export default function SkillPage() {
  const html = markdownToHtml(SKILL_MD);
  return (
    <section className="grid gap-6">
      <section className="accent-bar relative overflow-hidden rounded-lg border border-border bg-gradient-to-b from-surface/95 to-surface-strong/90 p-6 shadow-[0_24px_60px_rgba(0,0,0,0.4)]">
        <div className="mb-3 flex items-end justify-between gap-4 max-[720px]:grid max-[720px]:gap-2">
          <div>
            <p className="mb-2 font-mono text-xs font-bold uppercase tracking-[0.12em] text-accent [text-shadow:0_0_12px_rgba(245,158,11,0.25)]">AI Discovery</p>
            <h1 className="font-heading text-2xl leading-tight text-foreground">AgentPassport Skill</h1>
          </div>
          <div className="flex items-center gap-3">
            <a className="w-fit font-semibold text-accent transition-all hover:text-[#FBBF24] hover:[text-shadow:0_0_12px_rgba(245,158,11,0.3)] outline-none" href="/mcp">
              MCP Server
            </a>
